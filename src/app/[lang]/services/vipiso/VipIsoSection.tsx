...
interface VipIsoSectionProps {
  // 🌟 استلام كائن عادي مباشر ومفكوك مسبقاً من السيرفر لمنع انهيار الواجهة
  params: {
    lang: 'ar' | 'en';
  };
}

export default function VipIsoSection({ params }: VipIsoSectionProps) {
  // قراءة متغير اللغة مباشرة من الكائن المستقر
  ...
  const t = {
    ar: {
      ...
      subtitle: 'توثيق الحوكمة وبناء نظم إدارة الجودة والأمن السيبراني وفقاً لأعلى المعايير الدولية الصارمة لعام 2026 لتأمين الأصول الرقمية والمؤسسية واستحقاق المناقصات السيادية.',
      backBtn: 'العودة للمركز الرئيسي',
      ctaBtn: 'طلب تدقيق الامتثال وحزمة ISO عبر الواتساب',
      ...
      viewDetails: 'طلب التأهيل الفوري للحزمة',
    ...
    en: {
      ...
      viewDetails: 'Request Package Qualification',
  ...
  const whatsappUrl = `https://wa.me/971557983500?text=${encodeURIComponent(isRtl ? 'مرحباً جسور انترناشيونال، نود الاستفسار وطلب خدمات تأهيل وتدقيق منشأتنا للحصول على شهادات الأيزو القياسية ISO والامتثال الدولي لعام 2026' : 'Hello Josour International, I want to request a corporate ISO specification auditing session.')}`;
    ...
    >
      ...
      <section className="w-full max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-24 md:py-32 relative z-10 border-b border-brand-navy-dark/[0.03]">
        <div className="text-center space-y-4 mb-20">
          <h2 className="text-2xl md:text-4xl font-black text-brand-navy-dark text-center">{t.section2Title}</h2>
          <p className="text-xs md:text-sm font-semibold text-brand-navy-light/50 max-w-xl mx-auto text-center">{t.section2Desc}</p>
        </div>
      ...
      <section className="w-full max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-24 md:py-32 relative z-10 border-b border-brand-navy-dark/[0.03]">
        <div className="text-center space-y-4 mb-20">
          <h2 className="text-2xl md:text-4xl font-black text-brand-navy-dark text-center">{t.section3Title}</h2>
          <p className="text-xs md:text-sm font-semibold text-brand-navy-light/50 max-w-xl mx-auto text-center">{t.section3Desc}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 w-full">
          {t.isoCards.map((iso, idx) => (
            <Link 
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="bg-white/95 backdrop-blur-xl border border-brand-navy-dark/[0.04] p-8 md:p-12 rounded-[2rem] shadow-3xs flex flex-col justify-between items-start text-start relative overflow-hidden min-h-[480px] md:min-h-[460px] transition-all duration-500 md:hover:border-brand-gold/40 md:hover:shadow-2xl md:hover:-translate-y-1.5 group cursor-pointer"
            >
              ...
              <div className="w-full">
                ...
                <span className="text-[10px] font-bold tracking-wider text-brand-navy-light/40 uppercase bg-brand-light-bg px-3 py-1 rounded-md inline-block mb-4 border border-brand-navy-dark/[0.02] rtl:text-right ltr:text-left">
                  {iso.category}
                </span>
                
                <h3 className="text-2xl font-black text-brand-navy-dark mb-3 group-hover:text-brand-gold transition-colors duration-300 text-start w-full rtl:text-right ltr:text-left">
                  {iso.title}
                </h3>
                
                <p className="text-xs md:text-sm leading-relaxed font-medium text-brand-navy-light/70 mb-5 text-start w-full rtl:text-right ltr:text-left">
                  {iso.desc}
                </p>

                <div className="space-y-1 bg-brand-light-bg/50 p-4 rounded-xl border border-brand-navy-dark/[0.02] mb-6 text-start w-full rtl:text-right ltr:text-left">
                  <span className="text-[11px] font-bold text-brand-navy-dark/50 block text-start rtl:text-right ltr:text-left">
                    {t.scopeLabel}
                  </span>
                  <p className="text-xs font-medium text-brand-navy-light/80 leading-relaxed text-start rtl:text-right ltr:text-left">
                    {iso.scope}
                  </p>
                </div>
              </div>

              <div className="w-full flex items-center justify-between gap-3 text-[11px] font-bold text-brand-navy-dark/50 border-t border-brand-navy-dark/[0.04] pt-6 mt-10 group-hover:text-brand-navy-dark transition-colors duration-300 pb-3">
                <span className="tracking-wide text-start">{t.viewDetails}</span>
                <div className="w-8 h-8 rounded-full bg-brand-light-bg border border-brand-navy-dark/5 flex items-center justify-center group-hover:bg-brand-gold group-hover:border-brand-gold group-hover:text-brand-navy-dark transition-all duration-300 shrink-0 shadow-3xs">
                  {isRtl ? (
                    <ArrowLeft size={14} />
                  ) : (
                    <ArrowRight size={14} />
      ...
      <section className="w-full max-w-7xl mx-auto text-center py-12 border-t border-brand-navy-dark/[0.03] relative z-10">
